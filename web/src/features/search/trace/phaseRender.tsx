/**
 * Per-phase rendering of search-trace phases as `PipelineStages` rows.
 *
 ...
 * free-form `detail` map is read defensively (via `detailAccessors`): a missing
 * or wrong-typed key simply renders nothing rather than throwing, since
 * `detail` is wire JSON. Token / cost formatting lives in `costFormat`.
 *
 * Split out of the former monolithic 1181-line `phaseStages.tsx` (FE-01) so the
 * pure cost formatters (`costFormat`) and the defensive wire-JSON accessors
 * (`detailAccessors`) sit in their own unit-testable modules.
 *
 * Allowed deps: react, api/types, components/primitives (PipelineStages types,
 * Text), sibling costFormat/detailAccessors, own CSS module (§12.3).
 */

// rationale: over the §3.1 500-line ceiling. This is the residual per-phase
// RENDER layer after the two pure concerns (cost formatting, wire-JSON access)
// were extracted — nine phases, each with a one-line summary, a rich expandable
// body and a fallback detail node, plus the small structural helpers (query
// blocks, filter chips, kept/dropped filter rows) those bodies share. The phases
// are one cohesive surface kept in lock-step between the live rail and the folded
// trace; a file-per-phase split would scatter that shared vocabulary across many
// tiny modules without making any one easier to reason about. The scannability
// the ceiling protects comes here from the per-phase `switch`es and the section
// banners, not from a smaller line count.

import React from 'react';
import type { PhaseRecord, SearchPhase } from '../../../api/types';
import type {
  PipelineStage,
  PipelineStageState,
} from '../../../components/primitives/PipelineStages/PipelineStages';
import { formatCostLabel, ordinal } from './costFormat';
import {
  bool,
  fieldNum,
  fieldNumList,

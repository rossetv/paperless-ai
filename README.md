...
- **Search server** — serves a JSON API, a React web UI, and an [MCP](https://modelcontextprotocol.io) endpoint with two tools: `semantic_search` (free hybrid retrieval, no LLM calls — the calling agent synthesises its own answer) and `deep_search` (the full *agentic* pipeline: plan, retrieve, judge, and synthesise an answer).

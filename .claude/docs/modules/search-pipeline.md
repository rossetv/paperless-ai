...
| `src/search/planner.py` | `QueryPlanner` (`OpenAIChatMixin`). `plan()` = LLM call #1 → `RetrievalPlan | ClarifyNeeded`; `replan()` = the Phase-2 refinement re-plan, sharing the prompt/schema/parse path via `_complete_and_parse`. Fail-open to `build_broad_semantic_plan`. (490 lines, authorised rationale.) |
...
| `src/search/pricing.py` | `MODEL_PRICES` (single edit point for OpenAI list prices; `SEED_PRICES_AS_OF = "2026-06-10"`) and `price_call(model, provider, usage, table=...)`. Pure — no config, no I/O. |
...
| `tests/unit/search/` | One file per concern: `test_core*.py` (cache, fail_fast, identity, judge, skips, sources, trace), `test_planner*.py`, `test_retriever*.py`, `test_judge.py`, `test_synthesizer*.py`, `test_refinement.py`, `test_relevance.py`, `test_sources.py`, `test_fetch.py`, `test_cache.py`, `test_dates.py`, `test_text.py`, `test_prompts*.py`, `test_trace*.py`, `test_offload.py`, `test_pricing*.py`, `test_spend_quota.py`. |
...
- **Model fallback to `CLASSIFY_MODELS` only happens when the stage's provider equals `CLASSIFY_PROVIDER`** — otherwise those model names belong to a different endpoint and would 404. Same guard in `planner.py`, `judge.py` and `synthesizer.py`. `reasoning_effort` is passed only when the stage's provider is `openai`.
...
- **`core.py` (2133), `retriever.py` (945), `prompts.py` (909) and `planner.py` (490) carry explicit `# rationale:` comments** authorising their overage of the `CODE_GUIDELINES.md` §3.1 500-line ceiling — do not "fix" them by splitting.

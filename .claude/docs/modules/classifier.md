...
Run: `.venv/bin/python -m pytest tests/unit/classifier tests/integration/test_classifier_pipeline.py tests/e2e/test_classifier_workflow.py -q` (396 tests). The repo has a `.venv`; the system `python3` lacks the deps.
...
- **Error exits go through `common.tags.finalise_document_with_error`**, never a bare tag strip — `clean_pipeline_tags()` unconditionally removes `ERROR_TAG_ID` too (`worker.py:220-225`).
...
- **Provider gating.** `reasoning_effort` and the `json_schema` `response_format` are sent only when `CLASSIFY_PROVIDER == "openai"` (`provider.py::_build_params`); `max_tokens` only when `CLASSIFY_MAX_TOKENS > 0`. `temperature` is always requested and stripped by the shared compat layer if rejected.
...
- **`ClassificationResult` is genuinely immutable** — frozen, slots, `tags` a tuple, not a list — so results can cross threads (`result.py:14-27`).
...
- **Thread-safety claim vs. code (open question).** `daemon.py:167-172` asserts that "every one of [the taxonomy client's] accesses runs under the cache's RLock", but `TaxonomyCache._get_or_create_item_id` deliberately releases the lock before the HTTP create (`taxonomy.py:303-306`, "Step 2 — slow path: call the Paperless API without holding the lock"). With `DOCUMENT_WORKERS > 1`, one thread can be POSTing on the shared taxonomy httpx session while another holds the lock doing `refresh()`'s GETs on that same session. The comment overstates the guarantee; treat it as unresolved, not as a proven invariant.
- **Correspondent lookup is substring matching** (`allow_substring=True`, `taxonomy.py:93-96`): `"Revolut Ltd"` correctly finds `"Revolut"`, but the check is symmetric and unanchored (`normalised in key or key in normalised`), so a short correspondent name can bind to an unrelated longer existing one. Document types and tags use exact normalised matching.
...
- **`CLASSIFY_PRE_TAG_ID` defaults to `POST_TAG_ID`** (`src/common/config/_settings.py:608`), so a document that finishes OCR is picked up by the classifier automatically with no extra wiring.
...
- **`truncate_content_by_chars()` preserves the footer at the body's expense**: if the footer alone exceeds `CLASSIFY_MAX_CHARS` the body is dropped entirely (`content_prep.py:105-109`). The char cap is applied after page truncation; both notes are concatenated into the prompt.

...
|------|-------|----------------|
| Write | `StoreWriter(settings)` — `src/store/writer.py` | the `store_writer = StoreWriter(settings)` line in `src/indexer/daemon/_boot.py` (once; construction opens the connection and runs pending migrations) |
| Read | `StoreReader(settings)` — `src/store/reader/_reader.py`, re-exported from `store.reader` | the `_StoreReader(settings)` construction in `_resolve_components()` of `src/search/api.py` (lazy import, so `sqlite-vec` is not loaded eagerly) |
...
- **`StoreWriter.checkpoint()` is the one write-ish method that does NOT take `_write_lock`** — it runs `PRAGMA optimize` then `PRAGMA wal_checkpoint(TRUNCATE)` directly on the connection. It is called once per cycle from the indexer loop (the `store_writer.checkpoint()` call in `src/indexer/daemon/_loop.py`), where no other write is in flight.
- **`reader.keyword_document_search` returns an EMPTY page when `terms` is empty.** The "filter-only browse" case that `KeywordHit`'s docstring describes (`snippet=None`, `rank=0.0`) is constructed in the search layer (from the `list_documents` result in `src/search/core.py`), not by the store.

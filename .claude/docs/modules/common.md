...
| `src/common/config/_catalogue.py` | The key universe: `BOOTSTRAP_KEYS` (`APP_DB_PATH`, `INDEX_DB_PATH` — env-only, never in the DB), `SECRET_KEYS` (`OPENAI_API_KEY`, `PAPERLESS_TOKEN` — masked by the Settings API and by `Settings.__repr__`), `CONFIG_KEYS` (86 keys `PUT /api/settings` accepts), `REINDEX_KEYS` (`EMBEDDING_PROVIDER`, `EMBEDDING_MODEL`, `CHUNK_SIZE`, `CHUNK_OVERLAP` — a change warns the operator to re-index). |
...
| `src/common/config/_parsers.py` | Pure parse/validate/clamp helpers (`_get_int_env`, `_get_csv_env`, `_get_bool_env`, `_get_float_env`, `_resolve_*`). All fail closed with a `ValueError` naming the offending key. Blank/whitespace means "use the coded default" for **int and float only** — bool and CSV both reject it (see Gotchas). |
...
| `src/common/llm.py` | `OpenAIChatMixin` (retried `_create_completion`, `_create_with_compat` parameter-strip adaptation, `_complete_with_model_fallback`, thread-safe stats), the `_ClientRegistry` two-slot singleton `_openai_holder` (openai/ollama), `set_chat_client`, `LlmCallUsage`, `unique_models`, `extract_json_object`, `ThreadSafeStats`, `_STRIPPABLE_PARAMS`. |
...
| Gotcha | Detail |
|--------|--------|
| Blank-value handling is **inconsistent across the parsers**: int/float fall back to the coded default; bool and required-CSV raise; optional-CSV returns `[]`. | Only `_get_int_env` / `_get_float_env` treat blank as "use the default" (COMMON-20). `_get_bool_env({'X': ''}, 'X', True)` raises `ValueError: X must be a boolean value.` — a blanked bool key (`STALE_LOCK_RECOVERY`, `SEARCH_GATE_JUDGE`, …) hard-fails the next hot-reload. `_get_csv_env(..., require_non_empty=True)` raises `ValueError: OCR_MODELS must contain at least one model name.` — so does blanking `CLASSIFY_MODELS`. `OCR_REFUSAL_MARKERS` is parsed **without** `require_non_empty`, so blanking it returns `[]` silently — disabling OCR refusal detection with no error. |

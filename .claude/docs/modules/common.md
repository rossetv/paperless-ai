...
| `src/common/heartbeat.py` | `Heartbeat.beat()` / `beat_idle()` / `touch()` (upserts the daemon's `daemon_status` row, swallowing every `sqlite3.Error` / `OSError`; `touch` refreshes detail/freshness without writing the monotonic `processed_count` — the stall ticker's path), `run_heartbeat_ticker` for processes with no natural work cycle (the search server), and `run_stall_ticker` (beats via `touch` only while a poll cycle's `in_flight` event stays set — how a tag daemon parked on a flex capacity wait reads "working", not "stopped"). Imports `appdb.daemon_status` at module scope. |

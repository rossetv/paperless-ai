...
| `src/appdb/schema.py` | All DDL as `SCHEMA_V1`–`V5`, `V7`, `V8` string constants (no `V6` — that migration is data-only), plus `SCHEMA_VERSION: int = 8` (`src/appdb/schema.py`, `SCHEMA_VERSION`) and `ensure_schema(conn)`, which delegates to `migrations.run_migrations`. Tables: `meta`, `users`, `sessions` (v1), `recent_searches` (v2), `api_keys` (v3), `config` (v4), `daemon_status` + `reconcile_activity` (v5), `api_key_usage` (v7), `model_pricing` (v8). |
| `src/appdb/migrations.py` | Forward-only versioned runner. `MIGRATIONS` is the ordered `[(1,_migrate_v1) … (8,_migrate_v8)]` list (`src/appdb/migrations.py`, `MIGRATIONS`); `run_migrations` applies each pending migration inside its own explicit `BEGIN…COMMIT` with the `schema_version` bump in the same transaction. `_apply_schema_string` strips `--` comment lines then splits on `;` and executes statements one by one (`executescript` is avoided — it issues an implicit `COMMIT`). `AppDbError` is raised when the stored `schema_version` exceeds the highest known migration. `_read_schema_version` returns 0 only for a genuine "no such table" error; any other `OperationalError` propagates. |
...
| `src/appdb/api_keys.py` | `ApiKey` dataclass + `create` (`DuplicateKeyHashError`), `get_by_hash` (the bearer-auth lookup), `get_by_id`, `list_all`, `list_for_user`, `revoke` (soft delete, keeps history), `delete` (hard), `touch` (caller-throttled `last_used_at` + `request_count` bump), `update` (uses a module-private `_UNSET` sentinel, `src/appdb/api_keys.py` (`_UNSET`), so `expires_at=None` clears an expiry rather than meaning "unchanged"). Stores `sha256(key)` + a display prefix only. |
...
| Caller | Site | Shape |
|--------|------|-------|
| `search.appdb_setup.open_app_db` | `src/search/appdb_setup.py` (`open_app_db`), called from `src/search/api.py` (`create_app`) | `connect` + `ensure_schema` at server startup; the connection is closed in the same block (short-lived). |
| `common.config._loader` | `src/common/config/_loader.py` (`load_settings`, `_hot_load`) | Deferred imports, transient connection per load/hot-load — **this is the path every process takes to read config**, including the OCR/classifier/indexer daemons. |
| `search.deps.get_app_db` | `src/search/deps.py` (`get_app_db`) | Per-request connection: `connect` per request, closed in a `finally`. |
| Daemon heartbeat bootstrap | `src/ocr/daemon.py` (`main`), `src/classifier/daemon.py` (`main`), `src/indexer/daemon/_boot.py` (`_open_app_db`) | `connect` + `ensure_schema` once at daemon start; the connection is **long-lived** and handed to `common.heartbeat.Heartbeat` for the process's lifetime. |
...
- `SCHEMA_VERSION` (`src/appdb/schema.py`, `SCHEMA_VERSION`, currently **8**) must equal the last `MIGRATIONS` entry's version (`src/appdb/migrations.py`, `MIGRATIONS`). `run_migrations` compares the stored version against `MIGRATIONS[-1][0]` (`src/appdb/migrations.py`, `run_migrations`) and raises `AppDbError` when the stored version is higher — a database written by newer code fails loud rather than being re-migrated.
...
- **`config.seed_from_env` is idempotent** and only seeds when the config table is completely empty (`SELECT 1 FROM config LIMIT 1`). `src/common/config/_loader.py` (`_hot_load`, COMMON-04) depends on this: on a fresh deployment every process boots at once and all call `seed_from_env` concurrently. Do not relax that idempotency.

## Gotchas

- **Bug-shaped (verified by running it): `IntegrityError` is over-mapped.** `api_keys.create` maps **any** `sqlite3.IntegrityError` to `DuplicateKeyHashError` (`src/appdb/api_keys.py`, `create`), but `api_keys` also has a `FOREIGN KEY` on `owner_user_id`. `create(..., owner_user_id=999)` on a DB with no such user raises `DuplicateKeyHashError: an API key with this hash already exists`. Misleading, and would surface as a wrong HTTP status if any caller ever passed an unvalidated owner id. `users.create` has the same shape (`src/appdb/users.py`, `create`): any `IntegrityError` — including a CHECK violation on `role`/`status` — becomes `UsernameTakenError`.
...
- **`users.update` cannot clear a column to NULL**: `None` means "leave unchanged", so `email=None` is a no-op, not a clear. Documented as a known Wave-1 limitation in the docstring (`src/appdb/users.py`, `update`). `api_keys.update` solves the same problem properly with the `_UNSET` sentinel — the two writers are inconsistent on purpose.
...
- `recent_searches.record` uses `int(cursor.lastrowid or 0)` (`src/appdb/recent_searches.py`, `record`) — a `None` lastrowid silently yields id `0` instead of raising `RowVanishedError` as the sibling writers (users/sessions/api_keys) do.
- **`connect()` passes `check_same_thread=False`.** It is only safe because no connection is ever driven by two threads at once: the search server opens one per request (`search.deps.get_app_db`), its heartbeat thread opens its own (`src/search/api.py`, `_start_search_heartbeat`), and each daemon's long-lived connection is used only by that daemon's own loop. The comment in `src/appdb/connection.py` (`connect`) records that the previous shared-connection-on-`app.state` model made this flag a latent data-corruption bug. **Never reintroduce a connection shared across concurrent request threads.**
...
- `config.snapshot_config_with_version` issues a raw `BEGIN DEFERRED` (`src/appdb/config.py`, `snapshot_config_with_version`) and commits in a `finally`. It relies on the connection being in the `sqlite3` module's legacy autocommit-ish mode; a caller already inside a transaction would break it.
...
| Data-only migration | A `_migrate_vn` with no `SCHEMA_Vn` constant — see `_migrate_v6` (`src/appdb/migrations.py`) for the pattern. |
...
- Specs: the "web-redesign spec §5" the code cites throughout (config-in-database, the dashboard's `daemon_status`/`reconcile_activity`, api keys) is a historical design doc never committed to this repo (its legacy `docs/superpowers/specs/` location is gitignored and absent on disk). The only committed spec is `.claude/specs/20260715-flex-and-56-models.md`.

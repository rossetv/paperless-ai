...
| A lone `EMBEDDING_DIMENSIONS` change silently wipes the index | It is deliberately **not** a `REINDEX_KEY`, so the save is accepted with **no** re-index warning, and the running indexer never notices. `StoreWriter.check_embedding_model` compares the stored `(provider, model, dimensions)` meta against config and, on any mismatch, wipes + re-stamps the index (`src/store/writer.py::StoreWriter.check_embedding_model` → `_wipe_and_stamp_model`) — but it runs **only at indexer boot** (`src/indexer/daemon/_boot.py::main`, before the loop; never re-checked per cycle). So the wipe fires at the **next indexer restart** (redeploy/crash/manual restart), not at the next reconcile of the already-running daemon. Nothing in `validate_change_set` rejects it (`src/search/settings_service.py::validate_change_set`) | Only ever change it together with `EMBEDDING_MODEL`, to the width that model emits |

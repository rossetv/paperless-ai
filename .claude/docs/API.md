...
|---------------|------|---------|
| `GET /api/healthz` | public | `routes.py` (`healthz`) |
| `GET /api/setup/status`, `POST /api/setup` | public (setup token) | `account_routes.py` (`setup_status`, `setup`) |
| `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me` | public / session | `account_routes.py` (`login`, `logout`, `auth_me`) |
| `GET /api/stats/public` | public | `account_routes.py` (`stats_public`) |
| `POST /api/search` | readonly + `api` | `routes.py` (`search`) |
| `POST /api/search/stream` (NDJSON) | readonly + `api` | `routes.py` (`search_stream`) |
| `GET /api/facets`, `GET /api/stats`, `GET /api/documents` | readonly + `api` | `routes.py` (`facets`, `stats`, `documents`) |
| `POST /api/reconcile` | member + `api` | `routes.py` (`reconcile`) |
| `GET /api/documents/{id}` | readonly + `api` | `document_routes/_documents.py` (`document_summary`) |
| `GET /api/documents/{id}/pdf`, `/thumb` | readonly + `api` | `document_routes/_proxy.py` (`document_pdf`, `document_thumb`) |
| `GET /api/recent-searches` | readonly + `api` | `document_routes/_documents.py` (`recent_searches`) |
| `PATCH /api/documents/{id}` | member + `api` | `document_routes/_documents.py` (`patch_document`) |
| `POST /api/documents/{id}/reclassify`, `/retranscribe` (202) | member + `api` | `document_routes/_documents.py` (`reclassify_document`, `retranscribe_document`) |
...
| `fetch_documents` | no | Canonical full text from Paperless — **max 5 ids/call**, each capped at `FETCH_MAX_CHARS` (50 000, `core.py`) |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Every `/mcp` request fails with `RuntimeError: Task group is not initialized` | An ASGI sub-app attached by Route/mount never gets its lifespan run by the parent | `create_app`'s lifespan must call `mcp_app.session_manager.run()` (`src/search/api.py::create_app`, the `async with mcp_app.session_manager.run()` block) |

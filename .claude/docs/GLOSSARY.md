...
| `REFUSAL_MARK` | The fixed sentinel `CHATGPT REFUSED TO TRANSCRIBE` the transcription prompt tells a vision model to emit when it will not transcribe; also seeded into `OCR_REFUSAL_MARKERS` | `src/common/config/_settings.py:82`, `src/ocr/prompts.py` |
...
| Watermark | `modified_watermark` in `index.db.meta` — the Paperless `modified` instant the indexer has synced up to, minus `OVERLAP_MARGIN` (10 s) | `src/indexer/reconciler/_incremental.py:69` |
| Dead-letter | Dropping a document from the `failed_documents` map after `MAX_CONSECUTIVE_DOCUMENT_FAILURES` (5) consecutive index failures (logged CRITICAL); it is retried only when its content next changes | `src/indexer/reconciler/_failed_documents.py:43` |
...
| Chunk | A ~`CHUNK_SIZE`-char window (default 2000) of a document's text with `CHUNK_OVERLAP`; the unit of embedding, vector search and FTS. Hard-capped at 6000 chars (`_MAX_CHUNK_CHARS`) | `src/indexer/chunker.py:44` |
...
| `RetrievalPlan` / `RetrievalSpec` | The planner's output: one or more retrieval specs (mode `semantic` or `keyword`, plus filter guesses). On LLM failure it degrades to one broad semantic spec | `src/search/planner.py` |
| RRF | Reciprocal-rank fusion (`_RRF_K = 60`, 1-based ranks) merging vector and keyword ranks into one score | `src/search/retriever.py:67` |

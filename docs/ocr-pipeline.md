...
The OCR daemon reads the text off scanned documents so the rest of the system can work with words instead of pixels. It watches Paperless-ngx for documents you have tagged for OCR, sends each page through a vision-capable LLM, and writes the transcribed text back.

## In a nutshell

Paperless-ngx stores scans as images. This daemon turns those images into text. Every 15 seconds it asks Paperless "any documents tagged for OCR?", and for each one it finds:

1. Downloads the document and splits it into one image per page.
...
```mermaid
flowchart LR
    A[Paperless<br/>document tagged<br/>for OCR] --> B[Split into<br/>page images]
    B --> C[Transcribe each<br/>page via LLM]
    C --> D[Assemble<br/>document text]
    D --> E{Looks<br/>good?}
    E -- Yes --> F[Save text,<br/>tag as done]
...
A document moves through the daemon in five stages: **find it, fetch it, read it, assemble it, save it.** The rest of this section walks that path in order, then the deeper sections cover the internals — concurrency, memory, the model fallback chain, and what happens when something goes wrong.

### Finding work: the polling loop

The daemon polls Paperless every `POLL_INTERVAL` seconds (default: 15) for documents carrying the queue tag, `PRE_TAG_ID`. Tags are the daemon's entire notion of "what to do", so a document is skipped when it already carries one of these:

- `POST_TAG_ID` — already OCR'd. The stale `PRE_TAG_ID` is stripped automatically so the document leaves the queue cleanly.
- `OCR_PROCESSING_TAG_ID` — claimed by another worker instance (only when this lock tag is configured).
- `ERROR_TAG_ID` — previously failed.

Configuration is re-read at the top of every poll, so a saved change is picked up on the next cycle. The one exception is `POLL_INTERVAL` and `DOCUMENT_WORKERS`: they set the loop's cadence and pool size, so they are fixed for the loop's lifetime and only change on a restart.

**Source:** `src/common/document_iter.py`

### Fetching and splitting into pages

Once a document is selected, the daemon claims it (by adding `OCR_PROCESSING_TAG_ID`, if configured), downloads it, and converts it into page images — because the vision model sees images, not PDFs.

- **PDFs** are rasterised by Poppler into images, one per page.
- **Images** (JPEG, PNG, ...) are decoded directly by Pillow.
- **Multi-frame images** (e.g. a multi-page TIFF) are expanded into one image per frame, each treated as a separate page.
...
Each page is checked, encoded, and sent to a vision model. Blank pages are skipped without an API call; the rest are sent to the model with a system prompt that tells it to transcribe faithfully and nothing more. If the model refuses or errors, the daemon falls back to the next model in a configured chain. The detail of all three steps — blank detection, the prompt, and the fallback chain — is in the sections that follow.

### Assembling and saving

When every page has been transcribed, the per-page results are stitched into one document body, page headers and a model footer are added, and the text is checked against a set of **quality gates**. If it passes, the daemon writes the text back to Paperless and swaps `PRE_TAG_ID` for `POST_TAG_ID`. If it fails — empty output, an error marker, a refusal — the document is tagged as errored and removed from the queue so it is never silently retried into a loop.

---

## Image conversion: memory-bounded by design

Memory is the binding constraint for this daemon, not speed. It runs `DOCUMENT_WORKERS` documents at once, each fanning pages across `PAGE_WORKERS`, on a memory-capped container. A naive "decode the whole document into a list of images" approach would OOM-kill the container on a large scan. So the converter is built around two rules that keep peak memory (RSS) bounded:

- **Rasterise at the target size, not full DPI.** A PDF page rendered at 300 DPI is roughly 2480×3509 pixels (about 26 MB decoded) — but the model only ever sees it shrunk to `OCR_MAX_SIDE`. So Poppler is asked to scale each page's longer side to `OCR_MAX_SIDE` up front (`pdftoppm -scale-to`), and the bitmap that reaches RAM is roughly a quarter of the bytes a full 300-DPI render would cost. `OCR_DPI` (default: 300) is still passed so DPI-dependent hinting stays sensible, but the size cap drives the output dimensions.
- **Stream pages, never hold the whole document.** For PDFs, Poppler writes one image file per page into a temp directory, and the worker opens, OCRs, and deletes one page at a time. The whole document never sits in RAM; at most `PAGE_WORKERS` page bitmaps are resident at once. (Plain images are small and need no streaming, so they are decoded in one shot and held in memory.)

The temp directory's lifetime belongs to the worker, which releases it once the document is done — even if OCR raised partway through.

**Source:** `src/ocr/image_converter.py`

---

## Concurrency: pages and documents in parallel

Two levels of parallelism run at once:

- Within a document, pages are OCR'd in parallel by a thread pool of `PAGE_WORKERS` threads (default: 8).
- At the daemon level, up to `DOCUMENT_WORKERS` documents (default: 4) run concurrently.

So the daemon can have up to `PAGE_WORKERS × DOCUMENT_WORKERS` (default: 32) vision API calls in flight — bounded further by `LLM_MAX_CONCURRENT` if you set it.
...
`OCR_MODELS` is an ordered list of models. The first model is the **primary**; the rest are fallbacks (the list is deduplicated, preserving order). The idea is to let cheap, fast models handle most pages and reserve the more capable ones for the hard cases. For each page:
...
Per-document statistics are tracked: `attempts`, `refusals`, `api_errors`, and `fallback_successes` (a non-primary model carrying the page). They are logged once per document.
...
A **footer** — `Transcribed by model: …` listing the distinct models used, sorted — is appended whenever at least one page was transcribed. This footer is **load-bearing, not cosmetic**: the classification daemon later parses it and turns the model names into tags. (See `src/classifier/tag_filters.py`.)
